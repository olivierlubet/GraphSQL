  ...
  test("develop stars") {
    val g = fromSqlToGraphX(
      """
        ...
        | select id, a from baz ) bar
      """.stripMargin)
    //print(g)
    assert(areLinked(g, "bar.id", "foo.id"))
    assert(areLinked(g, "bar.a", "foo.a"))


    val catalog = new NFCatalog()
    List("create table bd.foo as select * from bd.bar", "select a,b from bd.bar")
      .foreach { sql =>
        val plan = Parser.parse(sql)
        CatalogBuilder(catalog).add(plan)
      }
    val g2 = GraphBuilder.buildFromCatalog(catalog)
    //print(g2)
    assert(areLinked(g2, "bd.bar.a", "bd.foo.a"))
  }
    ...
    test("UNION in FROM") {
      ...
      assert(areLinked(g, "id", "b.id"))
      assert(areLinked(g, "b.id", "a.foo.id"))
    }
